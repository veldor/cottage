...
/* @var $model \app\models\SingleHandler */
...
$form = ActiveForm::begin(['id' => 'singleHandler', 'options' => ['class' => 'form-horizontal bg-default'], 'enableAjaxValidation' => false, 'validateOnSubmit' => false]);
...
echo $form->field($matrix, 'payId', ['template' => '{input}'])->hiddenInput()->label(false);
echo $form->field($matrix, 'payedBefore', ['template' =>
    '<div class="col-lg-5">{label}</div><div class="col-lg-3">{input}{error}{hint}</div>'])
    ->textInput(['autocomplete' => 'off', 'type' => 'number', 'disabled' => true])
    ->hint('В рублях')
    ->label('Оплачено ранее');
